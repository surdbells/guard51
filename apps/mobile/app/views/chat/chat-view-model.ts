...
export class ChatViewModel extends Observable {
  ...
  async init(): Promise<void> {
    ...
    try {
      const res = await this.api.get('/chat/conversations');
      const convs = res.data?.conversations || [];
      if (convs.length) {
        this.conversationId = convs[0].id;
        const msgRes = await this.api.get(`/chat/conversations/${this.conversationId}/messages`);
  ...
  async onSend(): Promise<void> {
    ...
    if (!text?.trim()) return;
    try {
      await this.api.post(`/chat/conversations/${this.conversationId}/messages`, { content: text });
